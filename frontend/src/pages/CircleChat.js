...
function CircleChat({ token, circleId, onBack }) {
  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/messages/${circleId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f3f4f6' }}>
          ...
          messages.map((msg) => (
            ...
            >
              <div style={{ maxWidth: '70%' }}>
                ...
                <div
                  style={{
                    background: msg.user_id === currentUser.id ? '#3B82F6' : 'white',
                    color: msg.user_id === currentUser.id ? 'white' : '#1f2937',
                    padding: '10px 16px',
                    borderRadius: msg.user_id === currentUser.id ? '18px 18px 4px 18px' : '18px 18px 18px 4px',
      ...
      <form onSubmit={sendMessage} style={{ background: 'white', borderTop: '1px solid #e5e7eb', padding: '16px 24px', display: 'flex', gap: '12px' }}>
        <input
          ...
          style={{ flex: 1, padding: '12px', border: '1px solid #e5e7eb', borderRadius: '24px', outline: 'none' }}
        />
        <button
          type="submit"
          ...
          style={{
            padding: '8px 24px',
            background: inputMessage.trim() ? '#3B82F6' : '#9ca3af',
            color: 'white',
            border: 'none',
            borderRadius: '24px',
